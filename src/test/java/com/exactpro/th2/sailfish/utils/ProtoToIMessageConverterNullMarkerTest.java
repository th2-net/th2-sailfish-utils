...
class ProtoToIMessageConverterNullMarkerTest extends AbstractProtoToIMessageConverterTest {
    ...
    void convertsNullToMarker(boolean useDictionary) {
        ProtoToIMessageConverter converter = new ProtoToIMessageConverter(
                DEFAULT_MESSAGE_FACTORY, dictionaryStructure, ProtoToIMessageConverter.createParameters().setUseMarkerForNullsInMessage(true)
        );
        ...
        IMessage expected = DEFAULT_MESSAGE_FACTORY.createMessage("RootWithNestedComplex", converter.getNamespace());
        expected.addField("nullField", FilterUtils.NULL_VALUE);

        IMessage inner = DEFAULT_MESSAGE_FACTORY.createMessage("SubMessage", converter.getNamespace());
